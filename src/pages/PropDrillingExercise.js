...
const RootComponent = (props) => {
  ...
  const addProductToCart = (newProduct) => {
    // Tạo một danh sách sản phẩm mới từ giỏ hàng hiện tại
    const newProductList = cart.products.map((cartProduct) => {
      // Kiểm tra nếu sản phẩm trong giỏ hàng có cùng id với sản phẩm mới được thêm vào
    ...
    // Cập nhật giỏ hàng với danh sách sản phẩm và tổng tiền mới
  ...
  const removeProductFromCart = (removedProduct) => {
    ...
    const newTotalPrice = cart.totalPrice - removedProduct.price;
    setCart({ products: newProductList, totalPrice: newTotalPrice });
  };
  // Step 3
    ...
    <WrapperBox>
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item sm={6}>
          ...
          removedProduct={props.removedProduct}
          />
        </Grid>
        <Grid item sm={6}>
      ...
      <Grid container justifyContent="center">
        ...
        <Grid item xs={8}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Button variant="success" sx={{ width: "5rem" }} onClick={() => props.addProduct(props.product)}>
              Add
            </Button>
            <Button variant="error" sx={{ width: "5rem" }} onClick={() => props.removedProduct(props.product)}>
              Remove
            </Button>
          </div>
        </Grid>
      ...
      <Grid container justifyContent="center">
        ...
        <Grid item xs={8}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <Button variant="error" size="sm" style={{ width: "5rem" }} onClick={() => props.removedProduct(props.product)}>
              Remove
            </Button>
          </div>
        </Grid>
